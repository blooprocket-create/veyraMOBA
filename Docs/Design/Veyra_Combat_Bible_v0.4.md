...
### Shared lane-Spire and base-defense-tower aggro

**Corrupted lane Spires and both base-defense towers use the same targeting rules.** They normally prioritize hostile Fluxborn over hostile Vanguards.

When an enemy Vanguard damages an allied/defending Vanguard **within the tower's attack range**, the attacker draws **priority tower aggression** even while hostile Fluxborn are present. This includes owned DoT damage ticks (including a DoT applied earlier) when the owning Vanguard is in tower range **at the time of the damaging tick**. Owned companions/summons follow the same owner-attributed rule: their qualifying damage draws tower priority to their owning Vanguard if that owner is in range.

A tower **keeps attacking its current valid priority Vanguard target** even if another enemy Vanguard damages a defender or a hostile Fluxborn enters range. A subsequent attacker does not automatically steal priority. If the current target becomes invalid, the tower selects another eligible target by its normal priority and targeting policy.

Priority on a particular Vanguard ends when that Vanguard **leaves tower range, dies, or becomes untargetable**. **There is no lingering aggression mark that preserves priority after they step out of range.** Re-entering range does not automatically restore the previous Vanguard-priority state: the player must cause a **new qualifying damaging event against a defending Vanguard** to take priority away from hostile Fluxborn.

If no hostile Fluxborn are available when that Vanguard re-enters range, the tower may **target them normally** without waiting for them to attack a defender. Normal targeting is distinct from Vanguard-over-Fluxborn priority.

### Ramping tower damage

Consecutive tower shots against the **same Vanguard** deal escalating damage to that target, up to a configurable cap. The prior prototype stepping illustration is **+20% per consecutive shot, capped at five ramp stacks**:

100% → 120% → 140% → 160% → 180% → 200%

This is **prototype tuning data**, not a hardcoded formula or committed balance. The ramp resets when the tower **loses the target, switches target, the Vanguard leaves range, dies, or becomes untargetable**. When that Vanguard re-enters range, tower shots start a **new ramp**. No previous mark or timed memory restores old ramp stacks after leaving range.

Each tower owns its authoritative current-target/ramp state; do not use named-Vanguard or named-companion special cases, and do not share target/ramp state between separate towers.

### Backdoor protection and wave-driven structure combat

**Lane Spires, base-defense towers, and the Prime Well** gain backdoor damage mitigation when **no allied attacking-team Fluxborn are within that structure's editable protection radius**. Neither nearby attacking Vanguards nor their summons/companions substitute for Fluxborn.

When the last eligible allied Fluxborn leaves or dies, protection **ramps toward a tunable maximum over time**, not immediately at maximum strength. On a newly arriving eligible allied Fluxborn, backdoor protection **drops to zero immediately**. Damage already dealt is never undone. Protection mitigates incoming structure damage and does not override separate inhibitor/base-tower/Prime-Well **invulnerability prerequisites**.

**No tower Health regeneration** applies to lane Spires or base-defense towers. The passive Prime Well alone has its own Health regeneration **when all inhibitors are standing**, as defined in the Battleground Bible. At least one inhibitor down stops that regeneration; the Prime Well cannot be attacked unless **both** base towers have fallen and **at least one** inhibitor is currently down.

### Fluxborn aggression and siege priority

Ordinary Fluxborn prioritize opposing lane Fluxborn and advance toward structures. If an enemy Vanguard damages a nearby allied Vanguard, nearby Fluxborn can redirect their aggression to that attacker; once the attacker leaves their **editable engagement range**, they disengage and resume their normal lane targeting.

**Siege Fluxborn** prioritize enemy structures already within their attack range except when responding to eligible aggression against an allied Vanguard. Exact target-selection radii, threat memory and disengagement behavior belong to tunable gameplay data.
