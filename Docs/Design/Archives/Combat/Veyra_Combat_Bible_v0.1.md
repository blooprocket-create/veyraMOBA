...
A Vanguard dies when Health reaches 0 unless a specific death-prevention effect applies.

### Kill credit

Kill credit normally goes to the source of the damage event that reduces the target to 0.
...
A Death Prevention effect may prevent lethal damage and leave the Vanguard at 1 Health according to its own rule.

Multiple death-prevention effects resolve by explicit priority; they do not all trigger at once.
...
A Revive occurs after death has already happened.

Kill/assist credit and death triggers are not undone.

The Revive effect defines restored Health/resources and other revive-specific state.
...
11. Resolve lethal rules, Death Prevention, Execute, death, and attribution.
...
## 44. Death cleanup and revival state

Death is a hard cleanup boundary for ordinary temporary combat effects.
...
A Revive restores the Vanguard according to the Revive effect's Health/resource rules. It does not automatically restore the old temporary combat state.

Cooldowns continue ticking through death/revival by default unless explicitly overridden.

Permanent match progression persists.

Effects that persist through death must explicitly declare that behavior.
...
## 50. Open combat questions

The following are not yet locked and should not be invented by implementation:
...
- exact damage/CC behavior of future special-case mechanics not represented here.
