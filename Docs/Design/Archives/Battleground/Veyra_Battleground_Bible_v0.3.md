...
| **VERSION**       | 0.3 - Prototype Game Modes & Champion Select |
|-------------------|--------------------------------|
...
| **VICTORY**       | Destroying the enemy Prime Well severs its battlefield Attunement and ends the match.                                                           |
...
Shopping follows the familiar fountain-shop rhythm.

- Players must return to their own **Fountain** to buy or sell items.
- **Recall** returns the Vanguard to the Fountain after its channel completes.
- Shopping is not available remotely from the battlefield.
- The Fountain is the team's safe equipment-conversion space: earn resources on the map, decide when to leave, recall, convert gold into power, then return to play.
...
## 14. PROTOTYPE GAME MODES & CHAMPION SELECT

The first playable prototype supports two standard 5v5 queues:

### Casual Select

All players select Vanguards during the same champion-select phase.

- Hovered/selected-but-not-locked Vanguards are private to the player's own team.
- The enemy team cannot see what a player is hovering or considering.
- Once a Vanguard is **locked in**, that pick becomes visible to the enemy team.
- Lock-in is final for that player.
- There is no unlock, champion swap, or post-lock trade system in the prototype.
- Existing global uniqueness/no-mirror rules still apply unless changed by a later design decision.

The intent is to preserve simultaneous casual selection while still allowing teams to react to enemy picks that have already been committed.

### Draft Pick

Draft Pick uses a fixed alternating pick order.
...
Draft visibility follows the same lock rules as Casual Select:

- Hovers are hidden from the opposing team.
- Only locked picks are revealed.
- A locked pick cannot be unlocked.
- Players cannot swap locked Vanguards with teammates.

Ban phases are not defined by this version of the prototype rules and remain a separate design decision.
...
<thead>
<tr class="header">
<th><p><strong>15. FIRST-PLAYABLE GUARDRAILS</strong></p>
...
- Final base geometry around inhibitors and the Prime Well.
- Gold/XP sharing rules that preserve composition freedom.
